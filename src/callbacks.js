// ===================== EACH =====================

// Call iterator(value, key, collection) for each element of collection.
// Accepts both arrays and objects.
//
// Note: each() does not have a return value, but rather simply runs the
// iterator function over each item in the input collection.

each = function(collection, iterator) {

  for(var i in collection){
    //debugger
  	if(!isNaN(i)){ i = +i;}else if(i.length > 1){break;};
    iterator(collection[i],i,collection);
    }



};

// ===================== FILTER =====================

// Return all elements of an array that pass a truth test.

// Requirement: Implement your .each() function

filter = function(collection, test) {
  let array = [];
  for (let i = 0; i < collection.length; i++){
  	if(test(collection[i])){ //test is a function returning a boolean
      array.push(collection[i]);
  	}
  }
  return array;
};

// ===================== MAP =====================

// Return the results of applying an iterator to each element.

// map() is a useful primitive iteration function that works a lot
// like each(), but in addition to running the operation on all
// the members, it also maintains an array of results.

// Requirement: Implement your .each() function

map = function(collection, iterator) {
  let array = [];
  for(var i =0; i< collection.length; ++i){
  	array.push(iterator(collection[i])); //iterator is a function returning a new value taking one argument
  }
  return array;
};

// ===================== REDUCE =====================

// Reduces an array or object to a single value by repetitively calling
// iterator(accumulator, item) for each item. accumulator should be
// the return value of the previous iterator call.
//
// You can pass in a starting value for the accumulator as the third argument
// to reduce(). If no starting value is passed, the first element is used as
// the accumulator, and is never passed to the iterator. In other words, in
// the case where a starting value is not passed, the iterator is not invoked
// until the second element, with the first element as it's second argument.
//
// Example:
//   var numbers = [1,2,3];
//   var sum = reduce(numbers, function(total, number){
//     return total + number;
//   }, 0); // should be 6
//
//   var identity = reduce([5], function(total, number){
//     return total + number * number;
//   }); // should be 5, regardless of the iterator function passed in
//          No accumulator is given so the first element is used.

// Requirement: Implement your .each() function

reduce = function(collection, iterator, accumulator) {
  // Write your code here
    let value = 0;   
    for(let i = 0; i < collection.length; i++){
       if(accumulator != undefined){ //if accumulator exist then we add on the value
          value += iterator(accumulator,collection[i]); 
        }else{  //else we make value equal to the first element in the array
          if(i === 0){ 
          	value = collection[i];
          }else{ //skip the first iteration and then we make value return a new value each time
            value = iterator(value, collection[i]);
          }
        }   
    }
  return value;

  //MY RESOURCES 
  //http://adripofjavascript.com/blog/drips/boiling-down-arrays-with-array-reduce.html
  //https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce
};
